...
parser.add_argument("--src_trans", required=True,
                    help="Source translation at the previous iteration.")
parser.add_argument("--tgt_trans", required=True,
                    help="Target translation at the previous iteration.")
...
def binary_cross_entropy(x, y, smoothing=0, epsilon=1e-12):
    ...
    return -tf.reduce_mean(tf.log(x + epsilon) * y + tf.log(1.0 - x + epsilon) * (1 - y))   # noqa E501
...
max_time = tf.reduce_max([tf.shape(output[0])[1]
...
def build_train_op(global_step, encdec_variables, discri_variables):
    ...
    return tf.cond(
        ...
        false_fn=lambda: discri_opt.apply_gradients(discri_gradients, global_step=global_step))     # noqa E501
